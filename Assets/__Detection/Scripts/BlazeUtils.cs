...
public static class BlazeUtils
{
    ...
    public static Texture2D CropFaceToTexture(WebCamTexture srcTexture, Rect faceBox, int targetWidth, int targetHeight)
    {
        // Convert normalized rect (0..1) → pixel-space
        int x = Mathf.RoundToInt(faceBox.xMin * srcTexture.width);
        int y = Mathf.RoundToInt(faceBox.yMin * srcTexture.height);
        int w = Mathf.RoundToInt(faceBox.width * srcTexture.width);
        int h = Mathf.RoundToInt(faceBox.height * srcTexture.height);

        // Clamp to avoid going out of bounds
        x = Mathf.Clamp(x, 0, srcTexture.width - 1);
        y = Mathf.Clamp(y, 0, srcTexture.height - 1);
        w = Mathf.Clamp(w, 1, srcTexture.width - x);
        h = Mathf.Clamp(h, 1, srcTexture.height - y);

        // Get pixels from source
        Color[] pixels = srcTexture.GetPixels(x, y, w, h);

        // Create new texture and set pixels
        ...
        Graphics.ConvertTexture(cropped, resized); // GPU accelerated
